...
from ..tpcf_kernels.combine_fields import get_field_weighted_corr

# os.environ["OMP_NUM_THREADS"] = "1"
# %matplotlib inline


class MockAngularCov:
    def __init__(
        self,
        redshift,
        sm_thresh,
    ...
    def capture_field_footprint(self, area_deg2):
        ...
        for s in range(self.N_samples):
            ra0 = np.random.uniform(ra_min, ra_max - side)
            dec0 = np.random.uniform(dec_min, dec_max - side)
            ...
            in_patch_ran = (
                (self.ran_ra >= ra0)
                & (self.ran_ra <= ra0 + side)
                & (self.ran_dec >= dec0)
                & (self.ran_dec <= dec0 + side)
            )

            inside_polygons.append(in_patch)
            inside_polygons_ran.append(in_patch_ran)

        self.inside_polygons = inside_polygons
        self.inside_polygons_ran = inside_polygons_ran
        self.poly_locator_N = poly_locator_N
        self.mock_tot_area_deg2 = (ra_max - ra_min) * (dec_max - dec_min)

    def evaluate_field_tpcf(self):
        """
    ...
    def get_combined_corrs(self, sep, DDs, DRs, RRs, weights):
        corrs_combined = []

        for realization in range(0, len(DDs[0])):
            DD_realization_all_fields = []
            DR_realization_all_fields = []
            RR_realization_all_fields = []
            for field in range(0, len(weights)):
                DD_realization_all_fields.append(DDs[field][realization])
                DR_realization_all_fields.append(DRs[field][realization])
                RR_realization_all_fields.append(RRs[field][realization])
            (
                sep,
                DD,
                DR,

...
from hodinerva.experimental.defaults import DEFAULT_COSMOLOGY
from hodinerva.experimental.htools_mock.mock_angular_cov import MockAngularCov

if __name__ == "__main__":
    sys.path.append(os.path.abspath("/Users/kumail/minerva/SMDPL"))
    p = argparse.ArgumentParser()
    p.add_argument("--config", default="config_mock.yaml")
    args = p.parse_args()
    ...
    hod_runid = "5"
    N_samples = cfg["mock_N_samples"]
    hod_post = ascii.read(Path(cfg["base_path"]) / cfg["hod_post"])
    ...
    smdpl_z = np.load(Path(cfg["base_path"]) / cfg["smdpl_z"])

    zbin = cfg["zbin"]
    print(smdpl_z[zbin])

    redshift = smdpl_z[7]
    mthresh = mstar_thresh[zbin]
    zlabel = "z" + str(zbin + 1)

    simname = cfg["simname"]
    halo_finder = cfg["halo_finder"]
    ...
    sep = (theta_bin_edges[1:] + theta_bin_edges[:-1]) / 2
    ...
    for mth in range(0, len(mthresh)):
        ...
        mock.ra, mock.dec = mock.convert_xyz_to_radec(
            x=gals["x"], y=gals["y"], z=gals["z"], zbin=zbins[zbin]
        )
        mock_out["ra"] = mock.ra
        ...
        mock.ran_ra, mock.ran_dec = mock.convert_xyz_to_radec(
            x=ran_x, y=ran_y, z=ran_z, zbin=zbins[zbin]
        )
        ...
        DRs = []
        RRs = []
        area_deg2 = np.array(cfg["area_deg2"])
        for field in range(0, len(area_deg2)):
            # capture this field's footprint and store RA/DEC etc. in mock. so that mock.evaluate_tpcf can be run
            mock.capture_field_footprint(area_deg2[field])
        ...
        mock.get_combined_corrs(sep, DDs, DRs, RRs, area_deg2)

        mock_out["corrs_combined"] = mock.corrs_combined
        mock_out["Cov"] = mock.Cov

        out_dir = Path(cfg["base_path"]) / cfg["mock_out_dir"] / f"run{cfg['run_id']}"
        out_dir.mkdir(parents=True, exist_ok=True)

        np.save(
            out_dir / f"{zlabel}_{mlabel}_N{N_samples}",
            mock_out,
        )
